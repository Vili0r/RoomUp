<?php

use App\Models\User;
use Inertia\Testing\AssertableInertia as Assert;
use Spatie\Permission\Models\Role;
use Spatie\Permission\Models\Permission;
use Illuminate\Foundation\Testing\RefreshDatabase;

uses(RefreshDatabase::class);

it('redirects unauthenticated users')
    ->expectGuest()->toBeRedirectedFor('/admin/permissions/1', 'put');

it('fails if the user is normal user', function () {
    $permission = Permission::create(['name' => 'user management']);
    $user = User::factory()->create();
    
    actingAs($user)
    ->put(route('admin.permissions.update', $permission))
    ->assertStatus(403);
});

it('fails if the user is not admin', function () {
    Role::create(['name' => 'moderator']);
    $moderator = User::factory()->create()->assignRole('moderator');
    $permission = Permission::create(['name' => 'user management']);

    actingAs($moderator)
        ->put(route('admin.permissions.update', $permission))
        ->assertStatus(403);
});

it('renders the role edit page', function () {
    $role1 = Role::create(['name' => 'admin']);
    $role2 = Role::create(['name' => 'unknown user']);
    Permission::create(['name' => 'user management']);
    $user = User::factory()->create()->assignRole('admin');
    $role1->givePermissionTo('user management');
    
    // Act
    $response = actingAs($user)->get(route('admin.roles.edit', $role2));
    $response->assertStatus(200);
    // Assert
    $response->assertInertia(fn (Assert $page) => $page
        ->component('Admin/Role/Edit')
        ->has('role')
    );
});

it('validates the request details', function (){
    $role1 = Role::create(['name' => 'admin']);
    Permission::create(['name' => 'user management']);
    $user = User::factory()->create()->assignRole('admin');
    $role1->givePermissionTo('user management');
    $role2 = Role::create(['name' => 'unknown user']);

    actingAs($user)
        ->put(route('admin.roles.update', $role2))->assertSessionHasErrors([
            'name',
        ]);
});

it('stores the role', function () {
    uses(RefreshDatabase::class);
    $role1 = Role::create(['name' => 'admin']);
    Permission::create(['name' => 'user management']);
    $user = User::factory()->create()->assignRole('admin');
    $role1->givePermissionTo('user management');
    $role2 = Role::create(['name' => 'unknown user']);

    $response = actingAs($user)
        ->put(route('admin.roles.update', $role2), [
            'name' => 'unknown user updated',
        ]);

    $this->assertDatabaseHas('roles', [
        'name' => 'unknown user updated',
    ]);

    $response->assertRedirect();
});