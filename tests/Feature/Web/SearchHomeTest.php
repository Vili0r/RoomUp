...
it('renders the home search page with search request', function () {
    ...
    // Act
    $response = $this->get(route('home-search', ['search' => 'ipsa']));
    $response->assertStatus(200);
    
    // Assert
    $response->assertInertia(fn (Assert $page) => $page
        ->component('Home/HomeSearch')
        ->has('properties.data')
        ->has('selectedQueries', fn (Assert $page) => $page
            ->where('search', 'ipsa') // Assert that the search term is present in the filters
            ->etc()
        )
    );
});
