...
it('does not allow unauthenticated user to get the conversation', function() {
    $response = $this->post('/api/conversation/1/reply');

    $response->assertStatus(302);
});

it('saves a reply to a conversation', function () {
    ...
    $response = $this->post(route('api.conversation.reply', $conversation), $bodyReply);
