...
it('renders single room listing correctly', function () {
    ...
    $response = actingAs($user)->get('/api/property/room/' . $room->id);
...
it('renders single flat listing correctly', function () {
    ...
    $response = actingAs($user)->get('/api/property/flat/' . $flat->id);

    $response->assertStatus(200);
});

it('gives an error if the incorrect model is provided', function () {
    $user = User::factory()->create();

    $response = actingAs($user)->get('/api/property/shared/1');
