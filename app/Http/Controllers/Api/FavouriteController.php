...
class FavouriteController extends Controller
{
    ...
    {
        $flats = $user->favouriteFlats()->with(['address', 'advertiser', 'viewedUsers', 'favourites', 'availability'])->get()->map(fn($flat) => [
            'id' => $flat->id,
            'model' => 'flat',
            'title' => $flat->title,
            'cost' => $flat->cost,
        ...
        $rooms = $user->favouriteRooms()->with(['owner.address', 'owner.advertiser', 'viewedUsers', 'favourites'])->get()->map(fn($room) => [
            'id' => $room->id,
            'model' => 'room',
