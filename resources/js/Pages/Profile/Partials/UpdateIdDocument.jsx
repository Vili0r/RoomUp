...
export default function UpdateIdDocument({ className, user }) {
    ...
    const takePhoto = () => {
        ...
        setData("id_document", imageDataUrl);
    };
    ...
    const verifyPhotos = () => {
        ...
        (async () => {
            ...
            // detect a single face from the id image
            const idDocumentFacedetection = await faceapi
                .detectSingleFace(
                    imgRef2.current,
                    new faceapi.TinyFaceDetectorOptions()
                )
    ...
    const submitPhoto = () => {
        router.post("/profile-id_document-verification", data, {
            forceFormData: true,
            preserveScroll: true,
        });
    };
